...
顶部展示当前生效策略、上游尝试顺序和排除项，常用入口为“检查当前路由”和“探测当前路由”。拉取模型、探测全部模型和校验全部上游收进“批量操作”；手动输入上游的严格测试放在“高级诊断”。原有操作和路由用途保持不变。

账号编辑使用页面内存草稿，添加账号、显示密钥和页面内切换不会覆盖尚未保存的输入；刷新或关闭页面前会提醒保存。草稿不会额外写入浏览器持久存储。访问设置与 Command Code 分别保存，各自显示保存状态，保存其中一组不会提交另一组未保存的修改。

各页面显示网关连接状态和最后成功连接时间。连接失败与超过 30 秒未更新的数据有明确提示；已启用账号数量不再等同于连通性测试通过数量。
